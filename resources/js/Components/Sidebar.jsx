...
export default function Sidebar({ children }) {
    ...
    const [sidebarOpen, setSidebarOpen] = useState(true); // Nuevo estado para controlar la visibilidad en móvil

    return (
        <aside
            className={`h-screen ${sidebarOpen ? "block" : "hidden"} sm:block`}
        >
            <nav className="h-full flex flex-col bg-white border-r shadow-sm">
                <div className="p-4 pb-2 flex justify-between items-center">
                    ...
                    <div className="flex items-center">
                        ...
                        {/* Botón de abrir/cerrar en dispositivos móviles */}
                        <button
                            onClick={() => setSidebarOpen(!sidebarOpen)}
                            className="p-1.5 ml-2 text-gray-600 sm:hidden"
                        >
                            {sidebarOpen ? (
...
export function SidebarItem({ icon, text, active, alert, children }) {
    ...
    return (
        <li
            className={`relative flex flex-col items-start py-2 px-3 my-1
